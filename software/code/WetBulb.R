wetbulb <- function(TemperatureC,Pressure,Humidity,HumidityMode,ConvergenceMode){

  QSat_2 <- function(T_k, p_t){
    ...
    rs = epsilon * es_mb / (p0ndplam - es_mb + pracma::eps())
    ...
    foftk = ((Cf/T_k)^lambd_a)*(1 - es_mb/p0ndplam)^(vkp*lambd_a)* exp(-lambd_a*goftk);
    ...
    rs[rs>1]=NA
    rs[rs<0]=NA
    return(list(es_mb = es_mb, rs = rs, de_mbdT = de_mbdT, dlnes_mbdT = dlnes_mbdT, rsdT = rsdT, foftk = foftk, fdT = fdT))
  }
...
QSat_2.out <- QSat_2(TemperatureK, Pressure)
es_mb <- QSat_2.out$es_mb
rs <- QSat_2.out$rs
...
QSat_2.out <- QSat_2(Teq, Pressure)
es_mb_teq <- QSat_2.out$es_mb
rs_teq <- QSat_2.out$rs
de_mbdTeq <- QSat_2.out$de_mbdT
dlnes_mbdTeq <- QSat_2.out$dlnes_mbdT
rsdTeq <- QSat_2.out$rsdT
foftk_teq <- QSat_2.out$foftk
fdTeq <- QSat_2.out$fdT
...
while((max(delta)>.01 & iter<=maxiter)){
  QSat_2.out <- QSat_2(wb_temp+C, Pressure)
  
  es_mb_wb_temp <- QSat_2.out$es_mb
  rs_wb_temp <- QSat_2.out$rs
  de_mbdwb_temp <- QSat_2.out$de_mbdT
  dlnes_mbdwb_temp <- QSat_2.out$dlnes_mbdT
  rsdwb_temp <- QSat_2.out$rsdT
  foftk_wb_temp <- QSat_2.out$foftk
  fdwb_temp <- QSat_2.out$fdT  
...
fdwb_temp <- QSat_2.out$fdT
delta=(foftk_wb_temp - X)/fdwb_temp
subdo=which(abs(delta)>convergence)
...
QSat_2.out <- QSat_2(wb_temp[subdo]+C, Pressure[subdo])
es_mb_wb_temp <- QSat_2.out$es_mb
rs_wb_temp <- QSat_2.out$rs
de_mbdwb_temp <- QSat_2.out$de_mbdT
dlnes_mbdwb_temp <- QSat_2.out$dlnes_mbdT
...
#for www=subdo(:)'
#disp(sprintf('WARNING-Wet_Bulb failed to converge. Setting to T: WB, P, T, RH, Delta: %0.2f, %0.0f, %0.2f, %0.1f, %0.2g, %0.1f, %0.2g', [Twb(www), Pressure(www), TemperatureK(www), relhum(www), delta(www)]));
}
}
return(list(Twb = Twb, Teq = Teq, epott = epott))
}
